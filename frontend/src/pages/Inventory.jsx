...
export default function Inventory() {
  ...
  const save = async () => {
    try {
      ...
      toast.success("Product added"); setOpen(false); load();
      setForm({ sku: "", product_name: "", brand: "", category: "", cost_price: 0, selling_price: 0, mrp: 0, gst_rate: 18, stock: 0, warehouse: "" });
    ...
    <div className="p-6 lg:p-8">
        ...
        {isAdmin && (
          <div className="flex gap-2">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild><Button className="bg-rose-600 hover:bg-rose-700 rounded-sm" data-testid="new-product-btn"><Plus size={16} className="mr-1" />Add Product</Button></DialogTrigger>
      ...
      <div className="bg-white border border-zinc-200 overflow-x-auto" data-testid="inventory-table">
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.length === 0 && <tr><td colSpan="8" className="text-center py-12 text-zinc-400">No products. Add one or import a CSV.</td></tr>}
            {items.map((i) => (
